    ...
    <section className="py-24 lg:py-28 bg-[#0a0a0f] relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-5 gap-12 lg:gap-16 max-w-6xl mx-auto">
          ...
          >
            ...
            <div className="space-y-6">
              {contactCards.map((card, index) => (
                <motion.div
                  key={card.title}
                  className="glass rounded-xl p-6 card-hover border border-purple-500/20 hover:border-purple-400/40 group"
                  ...
                  whileHover={{ x: 5 }}
                >
                  <card.icon className="w-10 h-10 text-purple-400 mb-4 group-hover:text-purple-300 transition-colors" />
                  <h3 className="text-lg font-bold text-white mb-2">
                    {card.title}
                  </h3>
                  <div className="w-12 h-0.5 bg-gradient-to-r from-purple-500 to-cyan-500 mb-3" />
                  <p className="text-white font-medium">{card.primary}</p>
                  <p className="text-white/60 text-sm">{card.secondary}</p>
                  {card.tertiary && (
                    <p className="text-white/60 text-sm">{card.tertiary}</p>
                  )}
                  <a
                    ...
                    className="inline-flex items-center gap-2 text-purple-400 font-semibold mt-4 hover:gap-3 hover:text-purple-300 transition-all"
                  >
                    {card.action.label}
            ...
            >
              <motion.a
                ...
                className="w-12 h-12 bg-gradient-to-br from-purple-500 to-violet-600 rounded-xl flex items-center justify-center text-white hover:shadow-lg hover:shadow-purple-500/50 transition-shadow duration-300"
                aria-label="LinkedIn"
              ...
              <motion.a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-12 h-12 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl flex items-center justify-center text-white hover:shadow-lg hover:shadow-cyan-500/50 transition-shadow duration-300"
                aria-label="Facebook"
                whileHover={{ scale: 1.1, rotate: -6 }}
                whileTap={{ scale: 0.95 }}
              >
          ...
          >
            <div className="glass rounded-2xl p-8 lg:p-10 border border-purple-500/20">
              ...
              {isSubmitted ? (
                ...
                >
                  ...
                  <Button
                    onClick={resetForm}
                    className="btn-gradient btn-shine text-white"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleChange}
                      required
                      className="w-full px-4 py-3 bg-[#16161f] border-2 border-purple-500/20 rounded-lg text-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all appearance-none cursor-pointer"
                      style={{
                    ...
                    >
                      {serviceTypes.map((type) => (
                        <option key={type.value} value={type.value} className="bg-[#16161f] text-white">
                          {type.label}
                        </option>
                      ))}
                    </select>
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-white font-medium mb-2 text-sm">
                        Pickup Location <span className="text-purple-400">*</span>
                      </label>
                      <input
                        type="text"
                        name="pickupLocation"
                  ...
                  <div>
                    <label className="block text-white font-medium mb-2 text-sm">
                      Cargo Details <span className="text-purple-400">*</span>
                    </label>
                    <textarea
                      name="cargoDetails"
                      ...
                      required
                      rows={4}
                      className="w-full px-4 py-3 bg-[#16161f] border-2 border-purple-500/20 rounded-lg text-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all resize-y placeholder:text-white/40"
                  ...
                  <Button
                    ...
                    className="w-full btn-gradient btn-shine text-white py-6 text-lg font-semibold shadow-lg hover:shadow-xl hover:shadow-orange-500/30 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed group"
                  >
                    {isSubmitting ? (
                      <>
